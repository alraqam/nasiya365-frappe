...
class InstallmentPlan(Document):
    ...
    def validate_customer_limit(self):
        ...
        if self.is_new():
            ...
            if customer.status != "Active":
                frappe.throw(_("Customer is not active"))
            
            if self.principal_amount > customer.available_limit:
                frappe.throw(
                    _("Requested amount {0} exceeds available credit limit {1}").format(
    ...
    def update_progress(self):
        ...
        if self.schedule:
            self.paid_installments = len([s for s in self.schedule if s.status == "Paid"])
            self.overdue_installments = len([s for s in self.schedule if s.status == "Overdue"])
    ...
    def apply_payment(self, amount, payment_transaction=None):
        ...
        for installment in sorted_schedule:
            if installment.status in ["Pending", "Overdue", "Partial"]:
        ...
        if all(s.status == "Paid" for s in self.schedule):
