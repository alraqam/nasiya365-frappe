...
nasiya365.BnplControlCenter = class BnplControlCenter {
	...
	renderImeiSearch() {
		...
		const mag = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:18px;height:18px;flex:none;display:block"><circle cx="11" cy="11" r="7"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>`;
		const box = $(`
	...
	buildLegacySnapshot(kpis, overdueRows, dueRows, recent) {
		...
		const kpi_cards = [
			...
			{
				id: "revenue_mtd",
				// Не «выручка»: считаются полученные платежи, а не оформленные продажи.
				// Платёж по сделке прошлого месяца попадает сюда, а сегодняшняя
				// рассрочка — нет.
				label: __("Поступления (МТД)"),
				value: revenueMtd,
